...
DEFINE_COMPONENT_LOGGER("Network", "RetryManager")
...
bool RetryOperation::start() {
    if (m_running) {
        getLogger()->warning("Retry operation already running");
        return false;
    }
    m_running = true;
    m_cancelled = false;
...
bool RetryOperation::execute() {
    try {
        ...
        getLogger()->error("Exception in retry operation: {}", e.what());
        return false;
    } catch (...) {
        getLogger()->error("Unknown exception in retry operation");
        return false;
    }
}
void RetryOperation::scheduleRetry() {
    ...
    if (m_retryCount >= m_config.maxRetries) {
        getLogger()->debug("Maximum number of retries reached ({})", m_config.maxRetries);
        m_running = false;
    ...
    getLogger()->debug("Scheduling retry {} of {} in {} ms",
...
RetryManager::RetryManager()
    : m_defaultConfig() {
    getLogger()->debug("Created RetryManager");
}
RetryManager::~RetryManager() {
    cancelAll();
    getLogger()->debug("Destroyed RetryManager");
}
...
void RetryManager::cancelAll() {
    ...
    m_operations.clear();
    getLogger()->debug("Cancelled all retry operations");
}
