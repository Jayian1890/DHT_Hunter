...
DEFINE_COMPONENT_LOGGER("Network", "UDPReliability")
...
UDPReliabilityEnhancer::UDPReliabilityEnhancer(std::shared_ptr<Socket> socket, const UDPReliabilityConfig& config)
    ...
    getLogger()->debug("Created UDPReliabilityEnhancer with max retries: {}, initial timeout: {} ms",
...
bool UDPReliabilityEnhancer::start() {
    if (m_running) {
        getLogger()->warning("UDPReliabilityEnhancer already running");
        return false;
    }
    if (!m_socket) {
        getLogger()->error("No socket provided");
        return false;
    }
    m_running = true;
    // Start timeout thread
    m_timeoutThread = std::thread(&UDPReliabilityEnhancer::timeoutThread, this);
    getLogger()->debug("UDPReliabilityEnhancer started");
    return true;
}
void UDPReliabilityEnhancer::stop() {
    ...
    getLogger()->debug("UDPReliabilityEnhancer stopped");
}
bool UDPReliabilityEnhancer::isRunning() const {
    return m_running;
}
bool UDPReliabilityEnhancer::sendMessage(const uint8_t* data, int size, const network::EndPoint& endpoint,
                                        const std::string& transactionId,
                                        UDPResponseCallback responseCallback,
                                        UDPTimeoutCallback timeoutCallback,
                                        UDPErrorCallback errorCallback) {
    if (!m_running) {
        getLogger()->error("UDPReliabilityEnhancer not running");
        return false;
    }
    if (!m_socket) {
        getLogger()->error("No socket provided");
    ...
    if (size <= 0 || !data) {
        getLogger()->error("Invalid data");
        return false;
    }
    // Check if we're below the connection limit
    if (!m_throttler->requestConnection()) {
        getLogger()->warning("Connection limit reached, message not sent");
    ...
    if (m_rateLimiter) {
        if (!m_rateLimiter->request(static_cast<size_t>(size))) {
            getLogger()->warning("Rate limit reached, message not sent");
            // Release connection
    ...
    // Send the message
    auto udpSocket = std::dynamic_pointer_cast<UDPSocketImpl>(m_socket);
    if (!udpSocket) {
        getLogger()->error("Socket is not a UDP socket");
        return false;
    }
    if (udpSocket->sendTo(data, static_cast<size_t>(size), endpoint) <= 0) {
        auto error = m_socket->getLastError();
        getLogger()->error("Failed to send message: {}", m_socket->getErrorString(error));
        // Remove transaction
        {
            std::lock_guard<std::mutex> lock(m_transactionsMutex);
        ...
        // Call error callback if provided
        if (errorCallback) {
            errorCallback(actualTransactionId, static_cast<int>(error),
                         m_socket->getErrorString(error));
        }
        return false;
    }
    getLogger()->debug("Sent message with transaction ID: {}", actualTransactionId);
    return true;
}
bool UDPReliabilityEnhancer::processReceivedMessage(const uint8_t* data, int size, const network::EndPoint& endpoint) {
    if (!m_running) {
        getLogger()->error("UDPReliabilityEnhancer not running");
        return false;
    }
    ...
    if (size < 4) {
        getLogger()->error("Message too small to contain transaction ID");
        return false;
    }
    ...
    // Check for duplicate message
    {
        ...
        if (it != m_cachedTransactions.end()) {
            ...
            m_duplicateCount++;
            getLogger()->debug("Duplicate message detected with transaction ID: {}", transactionId);
    ...
    if (transaction && transaction->responseCallback) {
        ...
        // Release connection
        m_throttler->releaseConnection();
        getLogger()->debug("Processed response for transaction ID: {}", transactionId);
        return true;
    }
    ...
    getLogger()->debug("Received message with unknown transaction ID: {}", transactionId);
    return true;
}
void UDPReliabilityEnhancer::setConfig(const UDPReliabilityConfig& config) {
    ...
    getLogger()->debug("Updated configuration with max retries: {}, initial timeout: {} ms",
                 m_config.maxRetries, m_config.initialTimeout.count());
}
...
size_t UDPReliabilityEnhancer::getActiveTransactionCount() const {
    std::lock_guard<std::mutex> lock(const_cast<std::mutex&>(m_transactionsMutex));
...
void UDPReliabilityEnhancer::checkTimeouts() {
    ...
    for (auto& transaction : expiredTransactions) {
        ...
        // Release connection
        m_throttler->releaseConnection();
        getLogger()->debug("Transaction timed out: {}", transaction->transactionId);
    }
}
...
void UDPReliabilityEnhancer::retryTransaction(std::shared_ptr<UDPTransaction> transaction) {
    ...
    if (udpSocket->sendTo(transaction->data.data(), transaction->data.size(), transaction->endpoint) <= 0) {
        auto error = m_socket->getLastError();
        getLogger()->error("Failed to resend message: {}", m_socket->getErrorString(error));
        // Call error callback if provided
        if (transaction->errorCallback) {
            transaction->errorCallback(transaction->transactionId, static_cast<int>(error),
                                     m_socket->getErrorString(error));
        }
        ...
        getLogger()->debug("Retried transaction: {} (retry {}/{})",
