...
DEFINE_COMPONENT_LOGGER("Network", "ConnectionThrottler")

namespace dht_hunter::network {
ConnectionThrottler::ConnectionThrottler(size_t maxConnections)
    : m_maxConnections(maxConnections),
      m_activeConnections(0) {
    getLogger()->debug("Created ConnectionThrottler with max connections: {}", maxConnections);
}
...
void ConnectionThrottler::setMaxConnections(size_t maxConnections) {
    ...
    getLogger()->debug("Max connections updated to {}", maxConnections);
}
