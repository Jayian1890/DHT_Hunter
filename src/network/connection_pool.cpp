...
DEFINE_COMPONENT_LOGGER("Network", "ConnectionPool")
...
PooledConnection::PooledConnection(std::shared_ptr<Socket> socket, const EndPoint& endpoint)
    : m_socket(socket),
      ...
      m_isValid(true) {
    getLogger()->debug("Created pooled connection to {}", endpoint.toString());
}
...
bool PooledConnection::validate() {
    ...
    uint8_t buffer[1];
    int result = m_socket->receive(buffer, sizeof(buffer));
    // If the result is 0, the connection has been closed by the peer
    // If the result is negative, there was an error
    if (result == 0) {
        getLogger()->debug("Connection to {} has been closed by the peer", m_endpoint.toString());
        m_isValid.store(false);
        return false;
    }
    // Check for errors
    if (result < 0) {
        ...
        if (error == SocketError::WouldBlock ||
            error == SocketError::OperationInProgress || error == SocketError::NotConnected) {
            // These errors are expected and don't indicate a problem with the connection
            return true;
        }
        getLogger()->debug("Connection to {} is invalid: {}", m_endpoint.toString(),
...
ConnectionPool::ConnectionPool(const ConnectionPoolConfig& config)
    ...
    getLogger()->debug("Created connection pool with max connections: {}, max connections per host: {}",
...
std::shared_ptr<PooledConnection> ConnectionPool::getConnection(const EndPoint& endpoint,
    ...
    if (idleConnection) {
        ...
        if (m_config.validateConnectionsBeforeReuse && !idleConnection->validate()) {
            getLogger()->debug("Idle connection to {} is invalid, creating a new one", endpoint.toString());
        ...
        getLogger()->debug("Reusing idle connection to {}", endpoint.toString());
...
void ConnectionPool::returnConnection(std::shared_ptr<PooledConnection> connection) {
    ...
    getLogger()->debug("Returned connection to {} to the pool", connection->getEndpoint().toString());
}
void ConnectionPool::closeAll() {
    ...
    getLogger()->debug("Closed all connections");
}
...
void ConnectionPool::setConfig(const ConnectionPoolConfig& config) {
    ...
    getLogger()->debug("Updated connection pool configuration: max connections: {}, max connections per host: {}",
                 m_config.maxConnections, m_config.maxConnectionsPerHost);
}
...
std::shared_ptr<PooledConnection> ConnectionPool::createConnection(const EndPoint& endpoint,
    ...
    if (m_totalConnections.load() >= m_config.maxConnections) {
        getLogger()->debug("Maximum number of connections reached ({}), cannot create new connection to {}",
    ...
    if (getConnectionCountForHost(endpoint.getAddress()) >= m_config.maxConnectionsPerHost) {
        getLogger()->debug("Maximum number of connections per host reached ({}) for {}, cannot create new connection",
    ...
    if (!m_config.connectionThrottler->requestConnection()) {
        getLogger()->debug("Connection throttler denied connection to {}", endpoint.toString());
        return nullptr;
    }
    // Create a new socket
    auto socket = SocketFactory::createTCPSocket();
    if (!socket) {
        getLogger()->error("Failed to create TCP socket");
    ...
    if (!socket->connect(endpoint)) {
        ...
        if (error != SocketError::OperationInProgress && error != SocketError::WouldBlock) {
            getLogger()->error("Failed to connect to {}: {}",
        ...
        while (std::chrono::steady_clock::now() < endTime) {
            ...
            tv.tv_usec = static_cast<int>((remainingTime.count() % 1000) * 1000);
            ...
            if (result < 0) {
                // Error
                getLogger()->error("Select failed: {}", socket->getErrorString(socket->getLastError()));
                m_config.connectionThrottler->releaseConnection();
                return nullptr;
            } else if (result == 0) {
                // Timeout
                ...
                #else
                if (::getsockopt(socket->getHandle(), SOL_SOCKET, SO_ERROR,
                              &socketError, &errorLen) < 0) {
                #endif
                    getLogger()->error("Failed to get socket error: {}",
                                 socket->getErrorString(socket->getLastError()));
                    m_config.connectionThrottler->releaseConnection();
                    return nullptr;
                }
                if (socketError != 0) {
                    getLogger()->error("Connection to {} failed: {}",
                                 endpoint.toString(), socket->getErrorString(static_cast<SocketError>(socketError)));
                    m_config.connectionThrottler->releaseConnection();
                    return nullptr;
                }
        ...
        if (std::chrono::steady_clock::now() >= endTime) {
            getLogger()->error("Connection to {} timed out", endpoint.toString());
            m_config.connectionThrottler->releaseConnection();
            return nullptr;
        }
    }
    // Convert unique_ptr to shared_ptr and create a pooled connection
    std::shared_ptr<Socket> sharedSocket(socket.release());
    auto connection = std::make_shared<PooledConnection>(sharedSocket, endpoint);
    ...
    getLogger()->debug("Created new connection to {}", endpoint.toString());
    return connection;
}
    ...
    for (auto it = m_idleConnections.begin(); it != m_idleConnections.end();) {
        ...
        while (!connections.empty()) {
            ...
            if (idleTime > m_config.connectionIdleTimeout || lifetime > m_config.connectionMaxLifetime) {
                ...
                // Decrement the total connection count
                m_totalConnections--;
                getLogger()->debug("Removed expired connection to {}", connection->getEndpoint().toString());
            } else {
