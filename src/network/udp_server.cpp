...
bool UDPServer::start(uint16_t port) {
    auto logger = logforge::LogForge::getInstance().getLogger("Network.UDPServer");
...
int UDPServer::sendTo(const std::vector<uint8_t>& data, const std::string& address, uint16_t port) {
    auto logger = logforge::LogForge::getInstance().getLogger("Network.UDPServer");
