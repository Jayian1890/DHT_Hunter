...
namespace {
    auto logger = dht_hunter::logforge::LogForge::getLogger("Network.RateLimiter");
}
...
RateLimiter::RateLimiter(size_t bytesPerSecond, size_t burstSize)
    : m_bytesPerSecond(bytesPerSecond),
      ...
      m_availableTokens(burstSize == 0 ? bytesPerSecond * 2 : burstSize),
      m_lastUpdate(std::chrono::steady_clock::now()) {
    
    logger->debug("Created RateLimiter with rate: {} bytes/sec, burst size: {} bytes",
                 bytesPerSecond, m_burstSize.load());
}
...
void RateLimiter::setRate(size_t bytesPerSecond) {
    ...
    logger->debug("Rate limit updated to {} bytes/sec", bytesPerSecond);
}
...
void RateLimiter::setBurstSize(size_t burstSize) {
    ...
    logger->debug("Burst size updated to {} bytes", burstSize);
}
