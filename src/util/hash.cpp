...
namespace {
    auto logger = dht_hunter::logforge::LogForge::getLogger("Util.Hash");

    // SHA-1 implementation based on pseudocode from Wikipedia
    // https://en.wikipedia.org/wiki/SHA-1#SHA-1_pseudocode

    // Left rotate a 32-bit value by specified number of bits
...
void sha1(const uint8_t* data, size_t length, uint8_t* output) {
    ...
    for (int i = 0; i < 8; ++i) {
        paddedMessage[totalLen - 8 + i] = (bitLength >> (56 - i * 8)) & 0xFF;
    }
