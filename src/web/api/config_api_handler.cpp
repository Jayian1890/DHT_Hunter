...
network::HttpResponse ConfigApiHandler::handleGetConfig(const network::HttpRequest& request) {
    ...
    if (!m_configManager) {
        ...
        // Create error JSON
        json::JsonValue errorObj(json::JsonValue::createObject());
        errorObj.getObject()->set("error", json::JsonValue("Configuration manager is not initialized"));
    ...
    try {
        ...
        if (configJson.empty() || configJson == "{}") {
            ...
            // Create error JSON
            json::JsonValue errorObj(json::JsonValue::createObject());
            errorObj.getObject()->set("error", json::JsonValue("Failed to get configuration"));
            response.body = json::Json::stringify(errorObj);
        ...
        // Create error JSON
        json::JsonValue errorObj(json::JsonValue::createObject());
        errorObj.getObject()->set("error", json::JsonValue(std::string("Exception: ") + e.what()));
        response.body = json::Json::stringify(errorObj);
    }
...
network::HttpResponse ConfigApiHandler::handleGetConfigKey(const network::HttpRequest& request) {
    ...
    try {
        ...
        if (key.empty()) {
            ...
            // Create error JSON
            json::JsonValue errorObj(json::JsonValue::createObject());
            errorObj.getObject()->set("error", json::JsonValue("Key is required"));
            response.body = json::Json::stringify(errorObj);
        ...
        if (!m_configManager->hasKey(key)) {
            ...
            // Create error JSON
            json::JsonValue errorObj(json::JsonValue::createObject());
            errorObj.getObject()->set("error", json::JsonValue("Key not found: " + key));
            response.body = json::Json::stringify(errorObj);
        ...
        if (valueJson.empty()) {
            ...
            // Create error JSON
            json::JsonValue errorObj(json::JsonValue::createObject());
            errorObj.getObject()->set("error", json::JsonValue("Key not found or value is empty: " + key));
            response.body = json::Json::stringify(errorObj);
...
network::HttpResponse ConfigApiHandler::handlePutConfigKey(const network::HttpRequest& request) {
    ...
    try {
        ...
        if (body.empty()) {
            ...
            // Create error JSON
            json::JsonValue errorObj(json::JsonValue::createObject());
            errorObj.getObject()->set("error", json::JsonValue("Request body is required"));
            response.body = json::Json::stringify(errorObj);
        ...
        if (!m_configManager->setConfigValueFromJson(key, body)) {
            ...
            // Create error JSON
            json::JsonValue errorObj(json::JsonValue::createObject());
            errorObj.getObject()->set("error", json::JsonValue("Failed to set value for key: " + key));
            response.body = json::Json::stringify(errorObj);
        ...
        // Create success JSON
        json::JsonValue successObj(json::JsonValue::createObject());
        successObj.getObject()->set("success", json::JsonValue(true));
        successObj.getObject()->set("key", json::JsonValue(key));
...
network::HttpResponse ConfigApiHandler::handleReloadConfig(const network::HttpRequest& request) {
    ...
    try {
        ...
        if (!m_configManager->reloadConfiguration()) {
            ...
            // Create error JSON
            json::JsonValue errorObj(json::JsonValue::createObject());
            errorObj.getObject()->set("error", json::JsonValue("Failed to reload configuration"));
            response.body = json::Json::stringify(errorObj);
...
network::HttpResponse ConfigApiHandler::handleSaveConfig(const network::HttpRequest& request) {
    ...
    try {
        ...
        if (!success) {
            ...
            // Create error JSON
            json::JsonValue errorObj(json::JsonValue::createObject());
            errorObj.getObject()->set("error", json::JsonValue("Failed to save configuration"));
            response.body = json::Json::stringify(errorObj);
