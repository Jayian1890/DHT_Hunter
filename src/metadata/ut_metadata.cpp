...
DEFINE_COMPONENT_LOGGER("Metadata", "UTMetadata")
...
bool UTMetadata::processMessage(const uint8_t* data, size_t length,
    ...
    if (!data || length == 0) {
        getLogger()->error("Invalid ut_metadata message: empty data");
        return false;
    }
    ...
    try {
        ...
        for (size_t i = 0; i < length; ++i) {
            ...
            if (inString) {
                ...
                if (!inString) {
                    ...
                    inString = true;
                    stringLength = static_cast<size_t>(c - '0');
                }
            } else if (c == 'i' || c == 'l' || c == 'd') {
        ...
        if (dictEnd == 0) {
            getLogger()->error("Invalid ut_metadata message: could not find end of dictionary");
            return false;
        }
        ...
        if (!message || !message->isDictionary()) {
            getLogger()->error("Invalid ut_metadata message: not a dictionary");
            return false;
        }
        ...
        if (!msgType) {
            getLogger()->error("Invalid ut_metadata message: missing msg_type");
            return false;
        }
        ...
        if (!piece) {
            getLogger()->error("Invalid ut_metadata message: missing piece");
            return false;
        }
        ...
        switch (static_cast<UTMetadataMessageType>(*msgType)) {
            case UTMetadataMessageType::Request: {
                getLogger()->debug("Received ut_metadata request for piece {}", pieceIndex);

                if (requestCallback) {
                    auto pieceData = requestCallback(pieceIndex);
                    if (!pieceData.empty()) {
                        // We have the piece, send it
                        getLogger()->debug("Sending ut_metadata piece {}", pieceIndex);
                    } else {
                        // We don't have the piece, reject the request
                        getLogger()->debug("Rejecting ut_metadata request for piece {}", pieceIndex);
                    }
                }
                break;
            }
            ...
            case UTMetadataMessageType::Data: {
                getLogger()->debug("Received ut_metadata data for piece {}", pieceIndex);

                // Get total size
                auto totalSize = message->getInteger("total_size");
                if (!totalSize) {
                    getLogger()->error("Invalid ut_metadata data message: missing total_size");
                    return false;
                }
                ...
                if (m_metadataSize == 0) {
                    ...
                    getLogger()->warning("Metadata size mismatch: expected {}, got {}",
                                   m_metadataSize, metadataSize);
                }
                ...
                if (pieceSize != expectedSize) {
                    getLogger()->warning("Piece size mismatch for piece {}: expected {}, got {}",
            ...
            case UTMetadataMessageType::Reject: {
                getLogger()->debug("Received ut_metadata reject for piece {}", pieceIndex);
            ...
            default:
                getLogger()->warning("Unknown ut_metadata message type: {}", *msgType);
                return false;
        }

        return true;
    } catch (const bencode::BencodeException& e) {
        getLogger()->error("Failed to decode ut_metadata message: {}", e.what());
        return false;
    }
}
