...
DEFINE_COMPONENT_LOGGER("Metadata", "ExtensionProtocol")
...
bool ExtensionProtocol::processHandshake(const uint8_t* data, size_t length) {
    if (!data || length < 2) {
        getLogger()->error("Invalid extension handshake: too short");
        return false;
    }
    ...
    try {
        ...
        if (!handshake || !handshake->isDictionary()) {
            getLogger()->error("Invalid extension handshake: not a dictionary");
            return false;
        }
            ...
            for (const auto& [name, value] : extensions->getDictionary()) {
                if (value->isInteger()) {
                    ...
                    if (id > 0) {
                        ...
                        getLogger()->debug("Peer supports extension: {} (ID: {})", name, id);
                    }
                }
            }
        }
        ...
        if (metadataSize) {
            ...
            getLogger()->debug("Peer reported metadata size: {}", m_metadataSize);
        }
        // Process client version
        auto clientVersion = handshake->getString("v");
        if (clientVersion) {
            m_clientVersion = *clientVersion;
            getLogger()->debug("Peer client version: {}", m_clientVersion);
        }
        // Process peer port
        auto peerPort = handshake->getInteger("p");
        if (peerPort) {
            m_peerPort = static_cast<uint16_t>(*peerPort);
            getLogger()->debug("Peer listening port: {}", m_peerPort);
        }
        return true;
    } catch (const bencode::BencodeException& e) {
        getLogger()->error("Failed to decode extension handshake: {}", e.what());
        return false;
    }
}
...
std::vector<uint8_t> ExtensionProtocol::createExtensionMessage(
    ...
    if (extensionId == 0) {
        getLogger()->error("Cannot create extension message: extension {} not supported by peer",
                     extensionName);
        return {};
    }
    // Create the extension message
...
bool ExtensionProtocol::processExtensionMessage(const uint8_t* data, size_t length, uint8_t extensionId,
    ...
    if (extensionName.empty()) {
        getLogger()->warning("Received extension message with unknown ID: {}", extensionId);
        return false;
    }
    // Extract the payload
