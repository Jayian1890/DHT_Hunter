...
DEFINE_COMPONENT_LOGGER("Metadata", "TorrentFileBuilder")
...
std::shared_ptr<bencode::BencodeValue> TorrentFileBuilder::buildTorrentFile() const {
    if (!m_isValid) {
        getLogger()->error("Cannot build torrent file: Metadata is not valid");
        return nullptr;
    }
...
bool TorrentFileBuilder::saveTorrentFile(const std::string& filePath) const {
    ...
    getLogger()->info("Saved torrent file to {}", filePath);
    return true;
}

std::string TorrentFileBuilder::saveTorrentFile(const std::filesystem::path& directory) const {
    if (!m_isValid) {
        getLogger()->error("Cannot save torrent file: Metadata is not valid");
        return "";
    }
    ...
    if (!std::filesystem::exists(directory)) {
        if (!std::filesystem::create_directories(directory)) {
            getLogger()->error("Failed to create directory: {}", directory.string());
            return "";
        }
    }
...
bool TorrentFileBuilder::parseMetadata() {
    ...
    if (m_metadata.empty() || m_metadataSize == 0) {
        getLogger()->error("No metadata to parse");
        return false;
    }
    ...
    try {
        ...
        getLogger()->error("Failed to decode metadata: {}", e.what());
        return false;
    }
    ...
    try {
        ...
        getLogger()->debug("Calculated info hash: {}", util::bytesToHex(m_infoHash.data(), m_infoHash.size()));
    } catch (const std::exception& e) {
        getLogger()->error("Failed to calculate info hash: {}", e.what());
        return false;
    }
...
bool TorrentFileBuilder::extractInfoDictionary() {
    ...
    if (!infoDict.has_value() || infoDict->empty()) {
        getLogger()->error("Metadata does not contain an info dictionary");
        return false;
    }

    // Create a new BencodeValue for the info dictionary
    m_infoDict = std::make_shared<bencode::BencodeValue>(bencode::BencodeValue::Dictionary(*infoDict));

    // Get the name
    auto name = m_infoDict->getString("name");
    if (!name) {
        getLogger()->error("Info dictionary does not contain a name");
        return false;
    }
    ...
    if (!pieceLength) {
        getLogger()->error("Info dictionary does not contain a piece length");
        return false;
    }
    ...
    if (!pieces) {
        getLogger()->error("Info dictionary does not contain pieces");
        return false;
    }
    ...
    if (files) {
        ...
        for (const auto& fileValue : *files) {
            ...
            if (!length) {
                getLogger()->error("File dictionary does not contain a length");
                return false;
            }
            ...
            if (!pathList) {
                getLogger()->error("File dictionary does not contain a path");
                return false;
            }
        ...
        if (!length) {
            getLogger()->error("Info dictionary does not contain a length");
            return false;
        }
...
bool TorrentFileBuilder::validateInfoDictionary() const {
    ...
    if (!m_infoDict) {
        getLogger()->error("No info dictionary to validate");
        return false;
    }
    ...
    if (m_isMultiFile) {
        ...
        auto files = m_infoDict->getList("files");
        if (!files) {
            getLogger()->error("Multi-file torrent does not contain files");
            return false;
        }
        ...
        for (const auto& fileValue : *files) {
            ...
            if (!fileValue->getInteger("length")) {
                getLogger()->error("File does not contain a length");
                return false;
            }
            ...
            auto pathList = fileValue->getList("path");
            if (!pathList) {
                getLogger()->error("File does not contain a path");
                return false;
            }
        ...
        if (!m_infoDict->getInteger("length")) {
            getLogger()->error("Single-file torrent does not contain a length");
            return false;
        }
    }
