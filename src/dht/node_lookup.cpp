...
namespace {
    auto logger = dht_hunter::logforge::LogForge::getLogger("DHT.NodeLookup");
}

bool DHTNode::findClosestNodes(const NodeID& targetID, NodeLookupCallback callback) {
    if (!m_running) {
        logger->error("Cannot find closest nodes: DHT node not running");
        return false;
    }

    if (!callback) {
        logger->error("Cannot find closest nodes: No callback provided");
        return false;
    }
...
bool DHTNode::findPeers(const InfoHash& infoHash, GetPeersLookupCallback callback) {
    if (!m_running) {
        logger->error("Cannot find peers: DHT node not running");
        return false;
    }

    if (!callback) {
        logger->error("Cannot find peers: No callback provided");
        return false;
    }
...
bool DHTNode::announceAsPeer(const InfoHash& infoHash, uint16_t port, std::function<void(bool)> callback) {
    if (!m_running) {
        logger->error("Cannot announce as peer: DHT node not running");
        return false;
    }

    if (!callback) {
        logger->error("Cannot announce as peer: No callback provided");
        return false;
    }
    ...
    return findPeers(infoHash, [this, infoHash, port, callback](
        ...
        if (nodes.empty()) {
            logger->error("Cannot announce as peer: No nodes found");
            callback(false);
            return;
        }
        ...
        for (size_t i = 0; i < totalCount; ++i) {
            ...
            announcePeer(infoHash, port, token, node->getEndpoint(),
                ...
                [&completedCount, totalCount, callback](std::shared_ptr<ErrorMessage> error) {
                    // Failed announcement
                    completedCount++;
                ...
                [&completedCount, totalCount, callback]() {
...
bool DHTNode::startNodeLookup(const NodeID& targetID, NodeLookupCallback callback) {
    ...
    if (closestNodes.empty()) {
        logger->error("Cannot start node lookup: No nodes in routing table");
...
void DHTNode::continueNodeLookup(std::shared_ptr<NodeLookup> lookup) {
    // Check if the lookup has completed
    {
        ...
        if (lookup->iterations >= LOOKUP_MAX_ITERATIONS) {
            logger->debug("Node lookup reached maximum iterations");
    ...
    if (nodesToQuery.empty()) {
        ...
        if (lookup->activeQueries == 0) {
            logger->debug("Node lookup completed: no more nodes to query");
            lookup->completed = true;
            completeNodeLookup(lookup);
        }
    ...
    for (auto* node : nodesToQuery) {
        ...
        findNode(lookup->targetID, node->node->getEndpoint(),
            [this, lookup, node](std::shared_ptr<ResponseMessage> response) {
                ...
                if (findNodeResponse) {
                    // Handle the response
                    handleNodeLookupResponse(lookup, findNodeResponse, node->node->getEndpoint());
                }
            ...
            [this, lookup, node](std::shared_ptr<ErrorMessage> error) {
                logger->debug("Node lookup received error from {}: {} ({})",
            ...
            [this, lookup, node]() {
                logger->debug("Node lookup timed out for {}", 
                             node->node->getEndpoint().toString());
