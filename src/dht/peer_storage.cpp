...
namespace {
    auto logger = dht_hunter::logforge::LogForge::getLogger("DHT.PeerStorage");
}

bool DHTNode::storePeer(const InfoHash& infoHash, const network::EndPoint& endpoint) {
    if (!m_running) {
        logger->error("Cannot store peer: DHT node not running");
        return false;
    }
    ...
    // Store the peer
    {
        ...
        if (it != peerSet.end()) {
            ...
            logger->debug("Updated peer {} for info hash {}", endpoint.toString(), infoHashStr);
            return true;
        }
        ...
        if (peerSet.size() >= MAX_PEERS_PER_INFOHASH) {
            ...
            logger->debug("Removed oldest peer for info hash {} to make room for new peer", infoHashStr);
        }

        // Add the peer
        peerSet.insert(peer);
        logger->debug("Stored peer {} for info hash {}", endpoint.toString(), infoHashStr);
    }
...
void DHTNode::cleanupExpiredPeers() {
    ...
    if (removedCount > 0 || !emptyInfoHashes.empty()) {
        logger->debug("Cleaned up {} expired peers and {} empty info hashes", 
                     removedCount, emptyInfoHashes.size());
    }
}

void DHTNode::peerCleanupThread() {
    logger->debug("Peer cleanup thread started");
    ...
    logger->debug("Peer cleanup thread stopped");
}
