...
std::shared_ptr<Message> decodeMessage(const bencode::BencodeValue& value) {
    if (!value.isDictionary()) {
        getLogger()->warning("Message is not a dictionary");
        return nullptr;
    }
    // Check if it's a query, response, or error
    auto y = value.getString("y");
    if (!y) {
        getLogger()->warning("Message has no 'y' field");
        return nullptr;
    }
    if (*y == "q") {
        // Query
        ...
        getLogger()->warning("Unknown message type: {}", *y);
        return nullptr;
    }
}
std::shared_ptr<Message> decodeMessage(const std::string& data) {
    try {
        ...
        getLogger()->warning("Failed to decode message: {}", e.what());
        return nullptr;
    }
}
} // namespace dht_hunter::dht
