...
namespace {
    auto logger = dht_hunter::logforge::LogForge::getLogger("DHT.Types");
}
...
std::optional<NodeID> stringToNodeID(const std::string& str) {
    if (str.length() != 40) {
        logger->warning("Invalid node ID string length: {}", str.length());
        return std::nullopt;
    }
    ...
    for (size_t i = 0; i < 20; i++) {
        try {
            ...
            logger->warning("Invalid node ID string: {}", str);
            return std::nullopt;
        }
    }
...
TransactionID stringToTransactionID(const std::string& str) {
    if (str.length() % 2 != 0) {
        logger->warning("Invalid transaction ID string length: {}", str.length());
        return {};
    }
    ...
    for (size_t i = 0; i < id.size(); i++) {
        try {
            ...
            logger->warning("Invalid transaction ID string: {}", str);
            return {};
        }
    }
...
std::vector<CompactNodeInfo> parseCompactNodeInfo(const std::string& data) {
    ...
    if (data.length() % 26 != 0) {
        logger->warning("Invalid compact node info length: {}", data.length());
        return nodes;
    }
