...
bool ApplicationController::start() {
    ...
    try {
        ...
        dhtConfig.bootstrapNodes = m_configManager->getStringArray("dht.bootstrapNodes");
        dhtConfig.enableIPv6 = m_configManager->getBool("dht.enableIPv6", false);
        dhtConfig.enableNAT = m_configManager->getBool("dht.enableNAT", false);
        dhtConfig.enableUPnP = m_configManager->getBool("dht.enableUPnP", false);
        dhtConfig.enableDHTExtensions = m_configManager->getBool("dht.enableExtensions", true);

        // Create persistence manager
        m_persistenceManager = std::make_shared<dht::PersistenceManager>(dhtConfig.dataDirectory);
        ...
        auto statsService = m_dhtNode->getStatisticsService();
    ...
    try {
        ...
        if (m_dhtNode) {
            ...
            if (statsService && routingManager) {
                ...
                auto peerStorage = m_dhtNode->getPeerStorage();
                if (peerStorage) {
                    infoHashCount = peerStorage->getInfoHashCount();
                    peerCount = peerStorage->getPeerCount();
                }
                
                auto stats = statsService->getStatistics();
                ...
                ss << "BitScrape | Nodes: " << nodeCount
                   ...
                   << " | Peers: " << peerCount
                   << " | In: " << stats.bytesReceived / 1024 << " KB"
                   << " | Out: " << stats.bytesSent / 1024 << " KB";
...
void ApplicationController::registerEventHandlers() {
    ...
    eventBus->subscribe(unified_event::EventType::SYSTEM_SLEEP, [this](const unified_event::Event& event) {
        handleSystemSleep();
    });
    
    eventBus->subscribe(unified_event::EventType::SYSTEM_WAKE, [this](const unified_event::Event& event) {
        handleSystemWake();
    });
}
