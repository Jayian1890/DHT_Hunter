 ...
 */
using UDPResponseCallback = std::function<void(const uint8_t* data, int size, const EndPoint& endpoint)>;
...
public:
     ...
     */
    bool sendMessage(const uint8_t* data, int size, const EndPoint& endpoint,
     ...
     */
    bool processReceivedMessage(const uint8_t* data, int size, const EndPoint& endpoint);
