...
TEST_F(FormatterTest, NoPlaceholders) {
    std::string result = dl::Formatter::format("Hello, World!");
...
TEST_F(FormatterTest, MultiplePlaceholders) {
    std::string result = dl::Formatter::format("Hello, {}! Today is {} and the time is {}.",
...
TEST_F(FormatterTest, NumericValues) {
    std::string result = dl::Formatter::format("The answer is {}", 42);
    EXPECT_EQ(result, "The answer is 42");

    result = dl::Formatter::format("Pi is approximately {}", 3.14159);
...
TEST_F(FormatterTest, CustomObjects) {
    Point p{10, 20};
    std::string result = dl::Formatter::format("The point is at {}", p);
...
TEST_F(FormatterTest, ErrorCases) {
    // Unclosed brace
    std::string result = dl::Formatter::format("Unclosed brace {", "test");
    EXPECT_EQ(result, "Unclosed brace {");

    // Too few arguments
    result = dl::Formatter::format("Too {} few {} arguments", "many");
    EXPECT_EQ(result, "Too many few  arguments");

    // Too many arguments
    result = dl::Formatter::format("Too {} many arguments", "few", "extra", "arguments");
